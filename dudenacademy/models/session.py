...
class Session(models.Model):
    _name = 'dudenacademy.session'
    _description = 'dudenacademy.session'

    name = fields.Char('Name', required=True, index=True)
    course_id = fields.Many2one('dudenacademy.course', string='Course')
    ...
    taken_seats = fields.Float(
        string='Taken Seats', compute='_count_taken_seats', store=True)
        ...
        for rec in self:
            if rec.partner_id in rec.partner_ids:
                raise ValidationError(
                    "Your attendess is instructor: %s" % rec.partner_id.name)
